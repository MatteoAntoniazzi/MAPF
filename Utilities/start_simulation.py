...
def prepare_simulation(reader, frame, algorithm, independence_detection, map_number, solver_settings, n_of_agents):
    """
    ...
    """
    map = get_map(reader, map_number)
    agents = get_agents(reader, map_number, map, n_of_agents)
...
def get_map(reader, map_number):
    """
    Return the map object given the number of the choosen map.
    """
    print("Loading map...")
    map_width, map_height, occupancy_list = reader.load_map_file(MAPS_LIST.get(map_number))
    print("Map loaded.")

    return Map(map_height, map_width, occupancy_list)


def get_agents(reader, scene_number, map, n_of_agents):
    """
    ...
    agents = reader.load_even_scenario_file(EVEN_SCENES_LIST.get(scene_number), map.get_obstacles_xy(),
                                              map.get_width(), map.get_height(), n_of_agents=n_of_agents)
